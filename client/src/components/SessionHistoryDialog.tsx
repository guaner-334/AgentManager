...
export const SessionHistoryDialog: React.FC<SessionHistoryDialogProps> = ({
  ...
  // Initialize xterm and load content when viewingIndex changes
  useEffect(() => {
    ...
    const timer = requestAnimationFrame(() => {
      ...
      // sessions are displayed reversed, but the API uses original (chronological) index
      fetch(`/api/instances/${instanceId}/sessions/${viewingIndex}/content`)
        .then(r => {
          if (!r.ok) throw new Error('Failed to load');
          return r.text();
        })
        .then(content => {
          // Write in chunks to avoid overwhelming xterm parser
          const CHUNK = 64 * 1024;
          let offset = 0;
          const writeChunk = () => {
  ...
  if (viewingIndex !== null) {
    const reversedIdx = sessions.length - 1 - sessions.findIndex(
      (_, i) => (sessions.length - 1 - i) === viewingIndex
    );
    return (
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {sessions.map((session, idx) => {
                // Convert reversed display index back to original chronological index
